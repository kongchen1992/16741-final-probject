...
case 'shapenet'
    ...
    if ~exist(obj_dir, 'file')
        if ~exist(fullfile(ShapeNet_dir, class_uid), 'dir')
            mkdir(fullfile(ShapeNet_dir, class_uid));
        end
        unix(sprintf('scp -r bb9:/home/ci2cv/Datasets/ShapeNetCore.v1/%s/%s %s', ...
            class_uid, model_uid, fullfile(ShapeNet_dir, class_uid)));
    end
    object = loadawobj(obj_dir);
...
case 'pascal'
    ...
    if isempty(model_uid)
        ...
        for i = 1:numel(mat_content.(class_uid))
            ...
            % add anchors if exist
            anchor_names = anchor_of_cls(class_uid);
            ...
            if option.load_info
                model{i} = computeMeshInfo(model{i});
            end
        end
    else
